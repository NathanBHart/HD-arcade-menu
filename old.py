...
# Display Constants
DISPLAYSURF = pygame.display.set_mode((1200,1000), RESIZABLE)
...
# Set up fonts
text_huge = pygame.font.SysFont('Silkscreen', int(75 * IDEAL_PIXEL_RATIO), True)
...
class Card:
    ...
    def __init__(self, **kwargs):
        ...
        self.text_rect = (self.size[0] - DEFAULT_PADDING * 2, self.size[1] - DEFAULT_PADDING * 2,
                          self.location[0] + DEFAULT_PADDING, self.location[1] + DEFAULT_PADDING)
        self.display = pygame.image.load("resources/menu_ui/menu_card.png")
        self.display.fill((self.color[0], self.color[1], self.color[2], 100), special_flags=pygame.BLEND_ADD)
        self.display = pygame.transform.scale(self.display, (100, 100))
    ...
    def prepare_display(self):
        ...
        self.size = (CARD_WIDTH / ((abs(self.position)/DEPTH_SCALE) + 1),
                     CARD_HEIGHT / ((abs(self.position)/DEPTH_SCALE) + 1))
        self.location = ((DISPLAY_WIDTH - self.size[0]) / 2 + (CARD_WIDTH + DEFAULT_PADDING) * self.position,
                         (DISPLAY_HEIGHT - self.size[1]) / 2)
        self.rect = (self.size[0], self.size[1], self.location[0], self.location[1])
        self.text_rect = (self.size[0] - DEFAULT_PADDING * 2, self.size[1] - DEFAULT_PADDING * 2,
                          self.location[0] + DEFAULT_PADDING, self.location[1] + DEFAULT_PADDING)
        self.display = pygame.transform.scale(self.display, self.size)
...
class PrimaryMenuCard(Card):
    ...
    def __init__(self, **kwargs):
        ...
        display_text_box(self.title, self.text_rect, WHITE, text_huge, True, None, True, self.display)
...
menu_item = pygame.image.load("resources/menu_ui/menu_card.png")
...
while main_loop:
    ...
    DISPLAYSURF.blit(menu_item, (0, 0))

    display_text_box("HEllwapoiw;odjadjawdjalwdjaldkwnjdakldka", (50, 50, 1000, 1000), WHITE, text_huge, False)
